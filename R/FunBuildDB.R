...
################################################################################
# extract_ms2_from_target_list -------------------------------------------------

#' @title extract_ms2_from_target_list
#' @author Zhiwei Zhou
#' @description extract ms2 from target list and exported into MSP files
#' @param target_list_file a target list file. The target list file should include id, mz, rt
#' @param dir_path the directory path of the mzml files
#' @param polarity the polarity of the MS2 data, 'positive' or 'negative'. Default: 'positive'
#' @importFrom magrittr %>%
#' @importFrom crayon blue red yellow green bgRed
#' @importFrom stringr str_detect str_extract
#' @export
#' @examples
#' \dontrun{
#' extract_ms2_from_target_list(target_list_file = 'Targeted_MS2_List.xlsx',
#'                              dir_path = '~/Project/05_coworker/Kazuma/241014/demo_data',
#'                              polarity = 'positive')
#' }

# # please change the path to your own directory
# dir_path <- '~/Project/05_coworker/Kazuma/241014/demo_data'
# target_list_file <- 'Targeted_MS2_List.xlsx'
# polarity <- 'positive' # positive or negative

extract_ms2_from_target_list <- function(target_list_file,
                                         dir_path,
                                         polarity = c('positive', 'negative')) {

  message(crayon::blue("Read MS2 for mzml files\n"))
  # read MS2 for each mix file -------------------------------------------------
  ms2_file_list <- list.files(dir_path, pattern = 'mzML', recursive = TRUE)

  raw_ms2_spec <- lapply(ms2_file_list, function(x){
    retrieve_raw_ms2(file = x,
                     isolation_window = 'narrow',
                     dir_path = dir_path)
  })

  names(raw_ms2_spec) <- ms2_file_list

  dir.create(file.path(dir_path, 'extracted_ms2_spec'), showWarnings = FALSE, recursive = TRUE)
  save(raw_ms2_spec,
       file = file.path(dir_path, 'extracted_ms2_spec/raw_ms2_spec.RData'))



  # Modify the targeted list ---------------------------------------------------
  target_list <- readxl::read_xlsx(file.path(dir_path, target_list_file))
  colnames(target_list)[1:3] <- c('id', 'mz', 'rt')

  target_list <- target_list %>%
    dplyr::select(id, mz, rt, dplyr::everything()) %>%
    dplyr::mutate(rt = rt*60)

  # pairing ms2 with targeted list (grouping)
  ms2_spec_target <- lapply(names(raw_ms2_spec), function(x){
    # group ms2 according to target list
    result <- group_ms2(target_list = target_list,
                        ms2_data = raw_ms2_spec[[x]],
                        mz_tol = 20,
                        rt_tol = 15)
  })

  names(ms2_spec_target) <- ms2_file_list

  # save ms2_spec_target -------------------------------------------------------
  ms2_list_pos <- merge_ms2(ms2_spec_target)

  ms2_list_pos <- select_best_ms2(ms2_list = ms2_list_pos,
                                  precursor_mz_error_cutoff = 0.0025,
                                  precursor_rt_error_cutoff = 15,
                                  precursor_int_cutoff = 500,
                                  precursor_purity_cutoff = 0.5,
                                  is_keep_conflict_std = TRUE,
                                  rank_by = 'intensity_purity')

  save(ms2_list_pos,
       file = file.path(dir_path, 'extracted_ms2_spec', 'ms2_list_pos.RData'))


  # export the MSP files -------------------------------------------------------
  purrr::walk(seq_along(ms2_list_pos), function(i){
    # cat(i, ' ')
    temp_data <- ms2_list_pos[[i]]
    ...
    temp_info <- temp_data$info
    temp_spec <- temp_data$spec

    purrr::walk(seq_along(temp_info$id), function(j){
      generate_msp(file_name = file.path(dir_path, 'ms2_spec.msp'),
                   cmp_name = temp_info$id[j],
                   precusormz = temp_info$precursor_mz[j],
                   rt = temp_info$precursor_rt[j],
                   instrument_type = 'LC-ESI-qTof',
                   instrument = 'Agilent 6545 XF Q-TOF',
                   ce = temp_info$ce[j],
                   polarity = polarity,
                   spec = temp_spec[[j]])
    })
